...
const DimensionAnalystTable = (props) => {
  ...
  const fetchTableDataStats = () => {
    ...
    const apiURL = `${process.env.REACT_APP_SERVER_ADDRESS}/api/stats?job=DimAna&uid=${props.user.uid}&lt=${tableDataStats.lessThanDate}&gt=${tableDataStats.greaterThanDate}`
    ...
    fetch(apiURL).then((res) => res.json()).then((result) => {
      ...
      setTableDataStats(pre => ({
        ...pre,
        isLoading: false,
        data: [attempted, not_understandable, under_qa, minor, major, passed]
      }))
  ...
  const fetchTableData = () => {
    ...
    const apiURL = filterByQAStatus !== 'qa-status' ? `${process.env.REACT_APP_SERVER_ADDRESS}/api/super_table?job=DimAna&lt=${tableData.lessThanDate}&gt=${tableData.greaterThanDate}&page=${tableData.currentPage}&uid=${props.user.uid}&status=${filterByQAStatus}`
      : `${process.env.REACT_APP_SERVER_ADDRESS}/api/super_table?job=DimAna&lt=${tableData.lessThanDate}&gt=${tableData.greaterThanDate}&page=${tableData.currentPage}&uid=${props.user.uid}`
  ...
  const fetchByProductID = () => {
    ...
    const apiURL = `${process.env.REACT_APP_SERVER_ADDRESS}/api/super_table?job=DimAna&lt=${tableData.lessThanDate}&gt=${tableData.greaterThanDate}&page=${tableData.currentPage}&uid=${props.user.uid}&productID=${searchByID}`
    fetch(apiURL).then(res => res.json()).then((result) => {
      setTableData(pre => ({
        ...pre,
        isLoading: false,
  ...
  const searchItemByID = (e) => {
    ...
    if (e.target.value === '') {
      ...
      const apiURL = `${process.env.REACT_APP_SERVER_ADDRESS}/api/super_table?job=DimAna&lt=${tableData.lessThanDate}&gt=${tableData.greaterThanDate}&uid=${props.user.uid}`
      fetch(apiURL).then(res => res.json()).then((result) => {
        setTableData(pre => ({
          ...pre,
          isLoading: false,
          ...
          <tbody>
            ...
            {tableData.data.map((item, index) => (
              <tr key={index}>
                ...
                <td>{(item.status === null || item.status === 'under_qa') ? 'Under QA' : item.status === 'not_understandable' ? 'Not Understandable' : item.status === 'minor' ? 'MINOR [QA Passed]' : item.status === 'major' ? 'MAJOR [QA Passed]' : item.status === 'passed' ? '100% [QA Passed]' : 'N/A'}</td>
