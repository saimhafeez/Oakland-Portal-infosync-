...
function AllNads(props) {
    ...
    const fetchSuperAdminData = async (currentPage = tableData.currentPage) => {
        ...
        const lt = (new Date().getTime() / 1000).toFixed(0)
        const apiURL = `${process.env.REACT_APP_SERVER_ADDRESS}/api/super_table?job=QA-Extractor&lt=${lt}&gt=0&page=${currentPage}`
    ...
    const getAllProductsByFilter = () => {
        ...
        products = products.filter((item) => item.status === 'rejected_nad')
        return products
    }
