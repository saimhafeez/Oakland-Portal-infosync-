...
function AllNads(props) {
    ...
    const fetchSuperAdminData = async (currentPage = tableData.currentPage, pid = searchByID) => {
        ...
        const only_nads = '&extractor_status=rejected_nad'

        var apiURL = `${process.env.REACT_APP_SERVER_ADDRESS}/api/better_table?job=${tableFilter === 'Filter by Role' ? 'Extractor' : tableFilter}&lt=${tableData.lessThanDate}&gt=${tableData.greaterThanDate}&page=${currentPage}&items_per_page=${tableData.totalProductsPerPage}${only_nads}`
